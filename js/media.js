...
const logger = debug_1.default('waend:command:media');
const MEDIA_URL = config_1.default.public.mediaUrl;
...
function progress(length, name, options) {
    if (!options.progess) {
        ...
        options.progress.mediaName.appendChild(document.createTextNode(`${name} : `));
...
function uploadMedia() {
    ...
    const resolver = (resolve, reject) => {
        ...
        var handleFiles = files => {
            ...
            bluebird_1.default.each(files_array, single_uploader)
                .then(resolve)
                .catch(reject)
                .finally(() => {
                display.end();
