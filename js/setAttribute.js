...
const setUser = (ctx, components, key, value) => {
    const uid = components.user;
    return (ctx.binder
        .getUser(uid)
        .then((model) => ctx.binder.update(model, key, value)));
};
const setGroup = (ctx, components, key, value) => {
    const uid = components.user;
...
const setLayer = (ctx, components, key, value) => {
    const uid = components.user;
    const gid = components.group;
...
const setFeature = (ctx, components, key, value) => {
    const uid = components.user;
    const gid = components.group;
    const lid = components.layer;
